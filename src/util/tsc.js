import { Print as app } from '@/util/print';
    ...
    queryStatus.query = (buf) => {
      Taro.writeBLECharacteristicValue({
        ...
        success (res) {
          ...
          // app.BLEInformation.printStatus = true;
          Taro.showToast({
            icon: 'none',
            title: '打印机连接正常'
          });
        ...
        fail (err) {
          ...
          // app.BLEInformation.printStatus = false;

            Taro.showModal({
              content: '打印机失去连接，请重新连接后继续打印',
              success (res) {
                if (res.confirm) {
                  Taro.navigateTo({
                    url: '/pages/print/setting'
                  });
                }
              }
            });
